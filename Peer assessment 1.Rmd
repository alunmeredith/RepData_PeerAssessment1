...
Downloading, extracting and reading the data into a data frame called "data":  
...
```{r histogram date, fig.height=4, fig.width=6, fig.align='center'}
...
```
The summary statistics of this data is given below, showing the maximum number of steps taken as `r max(interval.steps$mean.steps)` at interval `r which.max(interval.steps$mean.steps)`
```{r}
...
=================================================================================
Seperate out the data into weekday and weekend measurements by creating a new factor variable with two levels "weekday" and "weekend" based on the "day" factor variable.

```{r}
imputed.data <- mutate(imputed.data, weekend = 
```